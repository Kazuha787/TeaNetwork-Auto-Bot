...
function loadAccounts() {
  try {
    ...
    if (!fs.existsSync(accountsFile)) {
      logWindow.log("{red-fg}[[ERROR]] accounts.txt NOT FOUND{/red-fg}");
      logWindow.log("{yellow-fg}Please create accounts.txt with private keys (one per line){/yellow-fg}");
      process.exit(1);
    }
    const accounts = fs.readFileSync(accountsFile, "utf-8")
      .split("\n")
      .map((key) => key.trim())
      .filter((key) => key && key.length === 64);
    
    if (accounts.length === 0) {
      logWindow.log("{red-fg}[[ERROR]] No valid private keys found in accounts.txt{/red-fg}");
      process.exit(1);
    }
    
    return accounts.map((privateKey, index) => ({
      id: index + 1,
      wallet: new ethers.Wallet(privateKey, provider)
    }));
  } catch (error) {
    logWindow.log(`{red-fg}[[ERROR]] Failed to load accounts: ${error.message}{/red-fg}`);
    process.exit(1);
  }
}
...
// Handle transactions
async function autoTransaction(wallet) {
  ...
  screen.render();
  setTimeout(() => autoTransaction(wallet), 86400000);
}
...
async function startProcess() {
  ...
  // Load accounts after UI initialization
  const accounts = loadAccounts();
  
  // Show available accounts
  logWindow.log("{magenta-fg}>>> AVAILABLE ACCOUNTS:{/magenta-fg}");
  accounts.forEach((account) => {
    logWindow.log(`{green-fg}${account.id}: ${account.wallet.address}{/green-fg}`);
  });
  
  const accountChoice = await getInput(">>> SELECT ACCOUNT NUMBER: ");
  const selectedAccount = accounts.find(acc => acc.id === parseInt(accountChoice));
  
  if (!selectedAccount) {
    logWindow.log("{red-fg}[[ERROR]] INVALID ACCOUNT SELECTION{/red-fg}");
    screen.render();
    setTimeout(startProcess, 3000);
    return;
  }
  
  await showWalletInfo(selectedAccount.wallet);
  ...
  if (choice === "1") {
    await deployContract(selectedAccount.wallet);
  } else if (choice === "2") {
    await autoTransaction(selectedAccount.wallet);
  } else if (choice === "3") {
    ...
    if (isNaN(parsedAmount) || parsedAmount <= 0) {
      ...
      await stakeTea(selectedAccount.wallet, parsedAmount);
    }
    setTimeout(startProcess, 3000);
  } else if (choice === "4") {
    ...
    if (isNaN(parsedAmount) || parsedAmount <= 0) {
      ...
      await withdrawTea(selectedAccount.wallet, parsedAmount);
    }
    setTimeout(startProcess, 3000);
  } else if (choice === "5") {
    await claimRewards(selectedAccount.wallet);
    setTimeout(startProcess, 3000);
  } else if (choice === "6") {
    ...
    if (isNaN(parsedAmount) || parsedAmount <= 0) {
      ...
      if (isNaN(parsedCount) || parsedCount <= 0) {
        ...
        await executeRandomTransfers(selectedAccount.wallet, parsedAmount, parsedCount);
      }
    }
    setTimeout(startProcess, 3000);
  } else if (choice === "7") {
    await executeDailyTask(selectedAccount.wallet);
    setTimeout(startProcess, 3000);
  } else {
